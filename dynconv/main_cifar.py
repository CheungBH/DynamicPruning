...
def main():
    ...
    parser.add_argument('--lr_decay', default=[150,250], nargs='+', help='learning rate decay epochs')
    ...
    # model
    parser.add_argument('--model', type=str, default='resnet32_BN', help='network model name')
    ...
    parser.add_argument('--mask_kernel', default=1, type=int, help='number of epochs')
...
def train(args, train_loader, model, criterion, optimizer, epoch, file_path):
    ...
    if file_path:
        with open(file_path, "a+") as f:
            logger.tick(f)
            f.write("Train: LR {}, gumbel temp {}, gumbel noise".format(
...
def validate(args, val_loader, model, criterion, epoch, file_path=None):
    ...
    with torch.no_grad():
        for input, target in tqdm.tqdm(val_loader, total=num_step, ascii=True, mininterval=5):
            ...
            if args.plot_ponder:
                if args.plot_save_dir and os.path.exists(
                        os.path.join(args.plot_save_dir, img_path[0].split("/")[-1])):
                    pass
                else:
                    viz.plot_image(input)
                    # viz.plot_ponder_cost(meta['masks'])
                    save_path = os.path.join(args.plot_save_dir,
                                             img_path[0].split("/")[-1]) if args.plot_save_dir else ""
