...
if args.dataset == 'cifar10':
    ...
    test_data=dset.ImageFolder(os.path.join(args.data_path, 'validation'), transforms.Compose([
        transforms.Scale(256), 
        transforms.CenterCrop(224),
        transforms.ToTensor(),
        normalize_imgnet,
...
if args.dataset=='imagenet':
    ...
    if args.arch=='resnet34':
        state_dict = torch.load(os.path.join(args.pretrain_path,'resnet34.pth'))
    elif args.arch=='resnet18':
        state_dict = torch.load(os.path.join(args.pretrain_path,'resnet18.pth'))
...
if args.dataset=='cifar10' and args.pretrain_path:
    if args.arch=='dyresnet20':
        pretrain_dict = torch.load(os.path.join(args.pretrain_path))
    elif args.arch=='dyresnet32':
        pretrain_dict = torch.load(os.path.join(args.pretrain_path))
    elif args.arch=='dyresnet56':
        pretrain_dict = torch.load(os.path.join(args.pretrain_path))
...
def train(train_loader, model, criterion, optimizer, epoch, log,
    ...
    with open(log_file, "a+") as log:
        log.write('**Train** Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 {error1:.3f}\n'.format(top1=top1, top5=top5,
                                                                                              error1=100 - top1.avg))
...
def validate(val_loader, model, criterion, log_file):
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with open(log_file, "a+") as log:
        log.write('**Test** Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Error@1 {error1:.3f}\n'.format(top1=top1,
                                                                                                   top5=top5,error1=100 - top1.avg))

    return top1.avg, top5.avg, losses.avg


def validate_ontrain(val_loader, model, criterion, log_file):
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
train_acc_1,train_los=validate_ontrain(train_loader, net, criterion, log_file)
...
for epoch in range(0, args.epochs):
    ...
    val_acc_1,val_acc_5, val_los_1 = validate(test_loader, net, criterion, log_file)
...
with open(log_file, "a+") as log:
    log.write('last acc:{}\n'.format(acc))
    log.write('best acc:{}'.format(recorder.max_accuracy(False)))